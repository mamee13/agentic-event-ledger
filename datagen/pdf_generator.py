...
def _m(v):
    return f"${v:,.0f}" if v and abs(v) >= 1 else ("—" if v is None else f"${v:.2f}")


def _neg(v):
    return f"(${abs(v):,.0f})" if v and v < 0 else _m(v)
...
def _doc(path, title, subtitle):
    ...
    elems = [
        Paragraph(
            f"<b>{title}</b>",
            ParagraphStyle("t", fontName="Helvetica-Bold", fontSize=14, spaceAfter=4),
        ),
        Paragraph(
            subtitle,
            ParagraphStyle(
        ...
        Paragraph(
            "(All amounts in USD — prepared in accordance with US GAAP)",
            ParagraphStyle(
                "g", fontName="Helvetica-Oblique", fontSize=8, spaceAfter=10, textColor=colors.gray
            ),
        ),
    ]
    return doc, elems
...
def generate_income_statement_pdf(
    company, fiscal_year: int, output_path: str, variant: str = "clean"
) -> None:
    ...
    if variant == "missing_ebitda":
        ...
        rows = [
            ["Line Item", f"FY{fiscal_year}", f"FY{fiscal_year - 1}"],
    ...
    if variant == "scanned":
        ...
        elems.append(
            Paragraph(
                "Note: Reproduced from original physical records. Some figures may reflect manual transcription.",
                ParagraphStyle(
                    "n", fontName="Helvetica-Oblique", fontSize=8, textColor=colors.gray
                ),
            )
        )
    doc.build(elems)
...
def generate_balance_sheet_pdf(
    ...
    fin = next(f for f in company.financials if f["fiscal_year"] == fiscal_year)
    doc, elems = _doc(
        output_path, company.name, f"Consolidated Balance Sheet — As of December 31, {fiscal_year}"
    )
    disc = random.uniform(500, 4500) if random.random() < 0.06 else 0
    eq_disp = fin["total_equity"] + disc
    rows = [
        ["Line Item", f"Dec 31, {fiscal_year}"],
        ["ASSETS", ""],
        ...
        [
            ...
            _m(
                max(
                    fin["total_liabilities"] - fin["current_liabilities"] - fin["long_term_debt"], 0
                )
            ),
        ],
    ...
    if disc > 0:
        ...
        elems.append(
            Paragraph(
                f"Note: Immaterial rounding adjustment of ${disc:,.0f} applied.",
                ParagraphStyle(
                    "n", fontName="Helvetica-Oblique", fontSize=8, textColor=colors.gray
                ),
            )
...
def generate_application_proposal_pdf(
    company, application_id: str, requested_amount: float, loan_purpose: str, output_path: str
) -> None:
    ...
    info = [
        ...
        ["Business Type:", company.legal_type],
    ...
    elems.extend(
        [
            ...
            Paragraph(
                "<b>Loan Request</b>",
                ParagraphStyle(
                    "h", fontName="Helvetica-Bold", fontSize=12, spaceAfter=6, textColor=NAVY
                ),
            ),
        ]
    )
    loan = [
        ["Requested Amount:", f"${requested_amount:,.0f}"],
    ...
    elems.extend(
        [
            ...
            Spacer(1, 0.2 * inch),
            Paragraph(
                "<b>Financial Highlights (FY2024)</b>",
                ParagraphStyle(
                    "h2", fontName="Helvetica-Bold", fontSize=12, spaceAfter=6, textColor=NAVY
                ),
            ),
        ]
    )
