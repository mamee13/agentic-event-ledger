...
from datagen.company_generator import TRAJECTORIES, generate_companies
    ...
    try:
        ...
        for c in companies:
            await conn.execute(
                ...
                c.address_city,
                c.address_state,
                date.fromisoformat(c.relationship_start),
                c.account_manager,
                c.risk_segment,
                c.trajectory,
            ...
            for fl in c.compliance_flags:
                await conn.execute(
                    ...
                    fl["severity"],
                    fl["is_active"],
                    date.fromisoformat(fl["added_date"]),
                    fl["note"],
                )
        pos_map: dict[str, int] = {}
        for stream_id, event_dict, recorded_at in all_events:
            pos = pos_map.get(stream_id, 0) + 1
            pos_map[stream_id] = pos
...
def main():
    ...
    print("=" * 60)
    print("APEX FINANCIAL SERVICES — DATA GENERATOR")
    print("=" * 60)
    ...
    risk_dist = {
        r: sum(1 for c in companies if c.risk_segment == r) for r in ["LOW", "MEDIUM", "HIGH"]
    }
    ...
    if not args.skip_docs:
        ...
        for i, c in enumerate(companies):
            d = Path(args.docs_dir) / c.company_id
            ...
            generate_application_proposal_pdf(
                ...
                str(d / "application_proposal.pdf"),
            )
            ...
            count += 5
            if (i + 1) % 20 == 0:
                print(f"  ... {i + 1}/{len(companies)} done")
    ...
    for target_state, count in SEED_SCENARIOS:
        for _ in range(count):
            company = (
                mt_company
                if target_state == "DECLINED_COMPLIANCE"
                else random.choice([c for c in companies if c.jurisdiction != "MT"])
            )
            app_id = f"APEX-{app_num:04d}"
            req = round(
                random.uniform(
                    company.financials[-1]["total_revenue"] * 0.08,
                    company.financials[-1]["total_revenue"] * 0.38,
                ),
                -3,
            )
            purpose = random.choice(company.loan_purposes)
            sim = EventSimulator(
                company=company, application_id=app_id, requested_amount=req, loan_purpose=purpose
            )
    ...
    print("  [OK] All events validated against EVENT_REGISTRY")
