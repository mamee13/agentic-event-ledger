...
# ruff: noqa: F403, F405, E402, N806, E501, I001
...
class EventSimulator:
    ...
    def _emit(self, stream_id: str, event: BaseEvent) -> None:  # noqa: F405
        cls = EVENT_REGISTRY.get(event.event_type)  # noqa: F405
        assert cls, f"Unknown event_type: {event.event_type}"
        # Validate by constructing
        cls(event_type=event.event_type, **event.to_payload())
    ...
    def _node(
        ...
        stream: str,
        session_id: str,
        agent_type: AgentType,  # noqa: F405
        name: str,
        seq: int,
        in_keys: list[str],
        out_keys: list[str],
        ...
        c = (
            cost
            if cost
            else (round((tok_in / 1e6 * 3.0 + tok_out / 1e6 * 15.0), 6) if llm else None)
        )
        self._emit(
            stream,
            AgentNodeExecuted(  # noqa: F405
                ...
                llm_cost_usd=c,
                duration_ms=random.randint(3000, 12000) if llm else random.randint(80, 500),
                executed_at=self._tick(seconds=random.randint(8, 40) if llm else 2),
            ),
        )

    def run(self, target_state: str) -> list[tuple[str, dict[str, Any], str]]:
        states = [
            "SUBMITTED",
            "DOCUMENTS_UPLOADED",
            "DOCUMENTS_PROCESSED",
        ...
        if idx <= 5 or target_state == "DECLINED_COMPLIANCE":
            return self.events
    ...
    def _step_submit(self):
        ...
        self._emit(
            ...
            ApplicationSubmitted(
                ...
                applicant_id=self.company.company_id,
        ...
        self._emit(
            ...
            DocumentUploadRequested(
                ...
                deadline=self.t + timedelta(days=7),
                requested_by="system",
            ),
        )
        self._emit(
            pkg,
            PackageCreated(
                package_id=self.application_id,
                application_id=self.application_id,
                required_documents=[
                    DocumentType.APPLICATION_PROPOSAL,
                    DocumentType.INCOME_STATEMENT,
        ...
        for doc_type, fmt, suffix in [
            ...
            self._emit(
                ...
                DocumentUploaded(
                    ...
                    file_hash=fh,
                    fiscal_year=2024 if doc_type != DocumentType.APPLICATION_PROPOSAL else None,
                    uploaded_at=self._tick(minutes=random.randint(1, 10)),
    ...
    def _step_doc_processing(self):
        ...
        self._emit(
            ...
            AgentSessionStarted(
                ...
                application_id=self.application_id,
                model_version=MODEL,
                langgraph_graph_version=LG_VERSION,
                context_source="fresh",
                context_token_count=random.randint(800, 1800),
                started_at=self._tick(seconds=3),
            ),
        )
        self._emit(
            astream,
            AgentInputValidated(
                ...
                validation_duration_ms=random.randint(50, 200),
                validated_at=self._tick(seconds=2),
            ),
        )
        self._node(
            astream,
            ...
            ["validated_inputs", "document_paths"],
        )
        ...
        for doc_type in [DocumentType.INCOME_STATEMENT, DocumentType.BALANCE_SHEET]:
            ...
            self._emit(
                ...
                DocumentFormatValidated(
                    ...
                    detected_format="pdf",
                    validated_at=self._tick(seconds=5),
                ),
            )
            self._emit(
                pkg,
            ...
            self._emit(
                ...
                AgentToolCalled(
                    ...
                    tool_duration_ms=random.randint(2000, 8000),
                    called_at=self._tick(seconds=random.randint(20, 60)),
                ),
            )
            facts = FinancialFacts(
                ...
                debt_to_ebitda=fin["debt_to_ebitda"],
                interest_coverage=fin["interest_coverage_ratio"],
                gross_margin=fin["gross_margin"],
                net_margin=fin["net_margin"],
                fiscal_year_end="2024-12-31",
                field_confidence={
                    "total_revenue": random.uniform(0.88, 0.98),
                    "net_income": random.uniform(0.82, 0.95),
                    "total_assets": random.uniform(0.88, 0.97),
                },
                page_references={
            ...
            self._emit(
                ...
                ExtractionCompleted(
                    ...
                    document_type=doc_type,
                    facts=facts,
                    raw_text_length=random.randint(2000, 6000),
                    tables_extracted=random.randint(2, 5),
                    processing_ms=random.randint(2000, 8000),
        ...
        self._emit(
            ...
            QualityAssessmentCompleted(
                ...
                overall_confidence=random.uniform(0.80, 0.96),
                is_coherent=True,
                anomalies=[],
                ...
                events_written=[
                    {
                        ...
                        "stream_position": len([e for e in self.events if e[0] == pkg]),
                    }
                ],
                output_summary="3 documents processed, high confidence. Package ready for credit analysis.",
                written_at=self.t,
            ),
        )
        self._emit(
            ...
            AgentSessionCompleted(
                ...
                total_llm_calls=1,
                total_tokens_used=random.randint(1000, 2200),
                total_cost_usd=cost_total,
                total_duration_ms=random.randint(8000, 25000),
        ...
        self._emit(
            ...
            CreditAnalysisRequested(
                application_id=self.application_id,
                requested_at=self._tick(seconds=5),
                requested_by=f"system:session-{sid}",
                priority="NORMAL",
            ),
        )
    ...
    def _step_credit(self):
        ...
        fin = self.company.financials[-1]
        score = (
            (1 - min(fin["debt_to_ebitda"] / 6, 1)) * 0.3
            + min(fin["current_ratio"] / 2, 1) * 0.25
            + (1 - min(fin["debt_to_equity"] / 3, 1)) * 0.25
            + (
                0.3
                if self.company.trajectory in ("GROWTH", "STABLE")
                else 0.1
                if self.company.trajectory == "DECLINING"
                else 0.2
            )
            * 0.2
        )
        risk_tier = (
            RiskTier.LOW if score > 0.70 else RiskTier.MEDIUM if score > 0.45 else RiskTier.HIGH
        )
        confidence = round(0.55 + score * 0.38, 2)
        limit = round(
            min(self.requested_amount, fin["total_revenue"] * 0.35) * random.uniform(0.80, 1.0), -3
        )
        ...
        self._emit(
            ...
            AgentSessionStarted(
                ...
                langgraph_graph_version=LG_VERSION,
                context_source="fresh",
                context_token_count=random.randint(1200, 2400),
                started_at=self._tick(seconds=3),
            ),
        )
        self._emit(
            ...
            AgentInputValidated(
                ...
                validation_duration_ms=random.randint(80, 250),
                validated_at=self._tick(seconds=2),
            ),
        )
        self._emit(
        ...
        for seq, (name, in_k, out_k, llm) in enumerate(
            [
                ...
                (
                    ...
                    ["applicant_id"],
                    ["historical_financials", "compliance_flags", "loan_history"],
                    False,
                ),
                (
                    "load_extracted_facts",
                    ["document_package_id"],
                    ["current_year_facts", "quality_flags"],
                    False,
                ),
                (
                    "analyze_credit_risk",
            ...
            if name == "load_applicant_registry":
                self._emit(
                    ...
                    AgentToolCalled(
                        ...
                        tool_duration_ms=random.randint(50, 300),
                        called_at=self._tick(seconds=1),
                    ),
                )
                self._emit(
                    credit,
                    HistoricalProfileConsumed(
                        application_id=self.application_id,
                        session_id=sid,
                        fiscal_years_loaded=[2022, 2023, 2024],
                        has_prior_loans=True,
                        has_defaults=False,
                        revenue_trajectory=self.company.trajectory,
                        data_hash=_sha(self.company.financials),
                        consumed_at=self.t,
                    ),
                )
        ...
        self._emit(
            ...
            CreditAnalysisCompleted(
                ...
                decision=CreditDecision(
                    risk_tier=risk_tier,
                    recommended_limit_usd=Decimal(str(limit)),
                    ...
                    else [f"Debt/EBITDA {fin['debt_to_ebitda']:.1f}x above optimal"],
                    data_quality_caveats=[],
                ),
                model_version=MODEL,
                model_deployment_id=f"dep-{uuid4().hex[:8]}",
                input_data_hash=_sha(
                    {"fin": self.company.financials, "req": self.requested_amount}
                ),
                analysis_duration_ms=random.randint(15000, 45000),
                completed_at=self._tick(seconds=10),
            ),
        )
        ...
        self._emit(
            ...
            AgentSessionCompleted(
                ...
                agent_type=AgentType.CREDIT_ANALYSIS,
                application_id=self.application_id,
                total_nodes_executed=7,
                total_llm_calls=1,
                total_tokens_used=total_tok,
                total_cost_usd=round(total_tok / 1e6 * 9.0, 5),
                total_duration_ms=random.randint(20000, 55000),
    ...
    def _step_fraud(self):
        ...
        self._emit(
            ...
            AgentSessionStarted(
                ...
                langgraph_graph_version=LG_VERSION,
                context_source="fresh",
                context_token_count=random.randint(900, 1800),
                started_at=self._tick(seconds=3),
            ),
        )
        self._emit(
            ...
            AgentInputValidated(
                ...
                validation_duration_ms=random.randint(50, 150),
                validated_at=self._tick(seconds=1),
            ),
        )
        self._emit(
            fraud,
        ...
        for seq, (name, llm) in enumerate(
            [
                ...
                ("load_document_facts", False),
                ("cross_reference_registry", False),
                ("analyze_fraud_patterns", True),
                ("write_output", False),
            ],
            1,
        ):
            self._node(
                ...
                seq,
                ["application_id"],
                ["fraud_assessment" if name == "analyze_fraud_patterns" else "data"],
                llm,
            )
        ...
        self._emit(
            ...
            FraudScreeningCompleted(
                ...
                recommendation=rec,
                screening_model_version=MODEL,
                input_data_hash=_sha(self.company.company_id),
                completed_at=self._tick(seconds=5),
            ),
        )
        ...
        self._emit(
            ...
            AgentSessionCompleted(
                ...
                total_tokens_used=total_tok,
                total_cost_usd=round(total_tok / 1e6 * 9.0, 5),
                total_duration_ms=random.randint(12000, 35000),
        ...
        self._emit(
            ...
            ComplianceCheckRequested(
                application_id=self.application_id,
                requested_at=self._tick(seconds=3),
                triggered_by_event_id=_sha(sid),
    ...
    def _step_compliance(self) -> bool:
        ...
        RULES = {
            "REG-001": (
                ...
                not any(
                    ...
                    for f in self.company.compliance_flags
                ),
                False,
            ),
            "REG-002": (
            ...
            "REG-004": (
                ...
                not (
                    self.company.legal_type == "Sole Proprietor" and self.requested_amount > 250_000
                ),
                False,
            ),
        ...
        self._emit(
            ...
            AgentSessionStarted(
                ...
                langgraph_graph_version=LG_VERSION,
                context_source="fresh",
                context_token_count=random.randint(600, 1400),
                started_at=self._tick(seconds=2),
            ),
        )
        self._emit(
            astream,
            AgentInputValidated(
                ...
                validation_duration_ms=random.randint(40, 150),
                validated_at=self._tick(seconds=1),
            ),
        )
        self._emit(
            ...
            if passes:
                ...
                self._emit(
                    ...
                    ComplianceRuleFailed(
                        ...
                        rule_name=name,
                        rule_version="2026-Q1-v1",
                        failure_reason=f"{name} check failed.",
                        is_hard_block=is_hard,
                        remediation_available=not is_hard,
                        evidence_hash=eh,
                        evaluated_at=self.t,
                    ),
                )
                ...
                events_written=[
                    {
                        ...
                        "stream_position": passed + failed + noted + 1,
                    }
                ],
                output_summary=f"Compliance: {verdict}. {passed} passed, {failed} failed.",
                written_at=self.t,
            ),
        )
        self._emit(
            ...
            AgentSessionCompleted(
                ...
                agent_type=AgentType.COMPLIANCE,
                application_id=self.application_id,
                total_nodes_executed=len(RULES),
                total_llm_calls=0,
                total_tokens_used=total_tok,
                total_cost_usd=0.0,
                total_duration_ms=random.randint(2000, 8000),
        ...
        if hard_block:
            self._emit(
                ...
                ApplicationDeclined(
                    application_id=self.application_id,
                    decline_reasons=[
                        f"Compliance hard block: {next(r for r, (n, p, h) in RULES.items() if not p and h)}"
                    ],
                    declined_by="compliance-system",
                    adverse_action_notice_required=True,
                    adverse_action_codes=["COMPLIANCE_BLOCK"],
                    declined_at=self._tick(seconds=5),
                ),
            )
        else:
            self._emit(
    ...
    def _step_decision(self, target_state: str):
        ...
        conf = round(
            random.uniform(0.70, 0.92)
            if rec == "APPROVE"
            else random.uniform(0.68, 0.88)
            if rec == "DECLINE"
            else random.uniform(0.45, 0.65),
            2,
        )
        amt = (
            Decimal(str(round(self.requested_amount * random.uniform(0.80, 1.0), -3)))
            if rec == "APPROVE"
            else None
        )
        ...
        self._emit(
            ...
            AgentSessionStarted(
                ...
                langgraph_graph_version=LG_VERSION,
                context_source="fresh",
                context_token_count=random.randint(2000, 4000),
                started_at=self._tick(seconds=3),
            ),
        )
        self._emit(
            ...
            AgentInputValidated(
                ...
                validation_duration_ms=random.randint(100, 300),
                validated_at=self._tick(seconds=2),
            ),
        )
        for seq, (name, llm) in enumerate(
            [
                ("validate_inputs", False),
                ("load_all_analyses", False),
                ("synthesize_decision", True),
        ...
        self._emit(
            ...
            DecisionGenerated(
                ...
                key_risks=[] if rec == "APPROVE" else ["Revenue trend requires monitoring"],
                contributing_sessions=[],
                model_versions={"orchestrator": MODEL},
                generated_at=self.t,
            ),
        )
        if rec == "APPROVE":
            self._emit(
                ...
                ApplicationApproved(
                    application_id=self.application_id,
                    approved_amount_usd=amt,
                    interest_rate_pct=round(random.uniform(5.5, 9.5), 2),
                    term_months=random.choice([24, 36, 48, 60]),
                    conditions=["Monthly financial reporting required"],
                    approved_by="auto",
                    effective_date=(datetime.now() + timedelta(days=3)).strftime("%Y-%m-%d"),
                    approved_at=self._tick(seconds=10),
                ),
            )
        elif rec == "DECLINE":
            self._emit(
                ...
                ApplicationDeclined(
                    application_id=self.application_id,
                    decline_reasons=["Insufficient debt service coverage", "High leverage"],
                    declined_by="auto",
                    adverse_action_notice_required=True,
                    adverse_action_codes=["HIGH_DEBT_BURDEN"],
                    declined_at=self._tick(seconds=10),
                ),
            )
        ...
        self._emit(
            ...
            AgentOutputWritten(
                ...
                application_id=self.application_id,
                events_written=[
                    {"stream_id": app, "event_type": "DecisionGenerated", "stream_position": 10}
                ],
                output_summary=f"Decision: {rec}. Confidence: {conf:.0%}.",
                written_at=self.t,
            ),
        )
        self._emit(
