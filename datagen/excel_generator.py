...
def generate_financial_excel(company, output_path: str) -> None:
    ...
    for col, w in [("B", 16), ("C", 16), ("D", 16)]:
        ws.column_dimensions[col].width = w
    for ci, h in enumerate(["Line Item", "FY 2022", "FY 2023", "FY 2024"], 1):
        _hdr(ws, 1, ci, h)
    IS_ROWS = [
    ...
    for ri, (label, key, bold, is_pct) in enumerate(IS_ROWS, 2):
        ...
        for ci, fin in enumerate(company.financials, 2):
            ...
            if is_pct:
                ...
                _money(ws, ri, ci, val, bold)
    # ── Sheet 2: Balance Sheet ─────────────────────────────────────────────────
    ws2 = wb.create_sheet("Balance Sheet")
    ws2.column_dimensions["A"].width = 34
    for col, w in [("B", 16), ("C", 16), ("D", 16)]:
        ws2.column_dimensions[col].width = w
    for ci, h in enumerate(["Line Item", "Dec 31 2022", "Dec 31 2023", "Dec 31 2024"], 1):
        _hdr(ws2, 1, ci, h)
    BS_ROWS = [
        ("ASSETS", None, True),
    ...
    for col, w in [("B", 14), ("C", 14), ("D", 14)]:
        ws3.column_dimensions[col].width = w
    for ci, h in enumerate(["Ratio", "FY 2022", "FY 2023", "FY 2024"], 1):
        _hdr(ws3, 1, ci, h)
    RATIOS = [
        ("LIQUIDITY", None, True),
        ...
        (
            "  Cash Ratio",
            lambda f: f["cash_and_equivalents"] / max(f["current_liabilities"], 1),
            False,
        ),
        ("LEVERAGE", None, True),
        ("  Debt/Equity", "debt_to_equity", False),
        ("  Debt/EBITDA", "debt_to_ebitda", False),
        ("  Interest Coverage", "interest_coverage_ratio", False),
