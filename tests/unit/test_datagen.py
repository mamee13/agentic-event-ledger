"""tests/unit/test_datagen.py — tests for the datagen simulator"""
# ruff: noqa: E501, N806
# Ported from starter/tests/test_schema_and_generator.py.
# Covers schema completeness, data generator correctness, and simulator trajectories.
...
# Ensure src is in path for ledger imports
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "src"))
# Ensure project root is in path for datagen imports
...
from ledger.schema.events import EVENT_REGISTRY, FinancialFacts
...
def test_event_registry_completeness() -> None:
    """All 40+ expected event types are registered."""
    required = [
...
def test_financial_facts_decimal_precision() -> None:
...
def test_companies_gaap_balance_sheet() -> None:
    ...
    for c in COMPANIES:
        for fin in c.financials:
            ...
            assert diff < 1.0, (
                f"{c.company_id} FY{fin['fiscal_year']}: balance sheet off by ${diff:.2f}"
            )


def test_companies_trajectory_distribution() -> None:
    """80 companies have the expected trajectory distribution."""
    cos = generate_companies(80)
    from collections import Counter

    d = Counter(c.trajectory for c in cos)
    assert d["GROWTH"] == 20, f"Expected 20 GROWTH, got {d['GROWTH']}"
    assert d["STABLE"] == 25
    assert d["DECLINING"] == 12


def test_montana_company_exists() -> None:
...
def test_simulator_approved_path() -> None:
...
def test_simulator_declined_compliance() -> None:
    ...
    if mt is None:
        cos = generate_companies(80)
        mt = next(c for c in cos if c.jurisdiction == "MT")
    ...
    assert "DecisionGenerated" not in types, (
        "DecisionGenerated must NOT appear after compliance hard block"
    )
...
def test_simulator_agent_session_sequence() -> None:
    ...
    for sid, types in sessions.items():
        assert types[0] == "AgentSessionStarted", (
            f"Session {sid}: first event must be AgentSessionStarted, got {types[0]}"
        )
        ...
        assert types[-1] == "AgentSessionCompleted", (
            f"Session {sid}: last event must be AgentSessionCompleted, got {types[-1]}"
        )


def test_schema_validator_catches_bad_event() -> None:
...
def test_all_seed_events_validate() -> None:
    ...
    for state, count in scenarios:
        for _ in range(count):
            company = (
                ...
                else random.choice([c for c in cos if c.jurisdiction != "MT"])
            )
